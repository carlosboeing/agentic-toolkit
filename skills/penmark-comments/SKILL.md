...
Review the document first, then decide where the findings go. Adding comments changes the file, so that decision belongs to the user unless they have already made it. Summaries, explanations, and extraction tasks do not activate this skill unless the user explicitly requests Penmark. Pull-request reviews and general code reviews do not activate this skill.
...
2. Resolve the output surface using the first rule that matches:
   ...
   5. Otherwise — print the full findings, then run the gate below.
   6. When asking is impossible, use the file if the target is tracked and clean in Git. Otherwise print the full findings and say why nothing was written.
...
Print the full findings first. Then ask using the harness question tool: `AskUserQuestion` on Claude Code, `ask_question` on Agy, `wait_user` or `ask_question` on Codex, native input on Cursor, plain bullets where none exists.
