...
| [`guide-revloop-credentials.md`](guide-revloop-credentials.md) | What each of revloop's keys, secrets and tokens is for **in CI** — local runs need none of them, only your `gh` login and a logged-in harness. Grouped by the four jobs they do: identity on GitHub, fetching the tool, logging in to a paid AI, and keeping that login alive. Covers why `GITHUB_TOKEN` is deliberately unused, why there are two GitHub Apps, and why the consuming repo's Deploy keys page is correctly empty. |
