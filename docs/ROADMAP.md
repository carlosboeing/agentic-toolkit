...
- **Advisory plan checker.** `skills/start-planning/scripts/check-plan.py` reports missing frontmatter, broken design links, duplicate task IDs, unresolved dependencies, tasks without verification evidence and absent design coverage, and says `unable to check` where a presentation cannot be parsed. `/start-planning` asks for a run before the draft is presented; 21 fixture tests run in CI. It never blocks a push.
