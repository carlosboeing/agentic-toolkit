...
- **Other**:
  ...
  - `scripts/sync-toolkit.sh` copies external skills only from directories listed in `EXTRA_SKILL_SOURCES` in local sync configuration. Add the CrossRev checkout's skills directory there to include `pr-review` and `pr-resolve`. It reports configured source directories that are missing. The same run writes OpenCode's `/`-menu command wrappers (one per hub skill) into `~/.config/opencode/command/` and copies `hooks/validate-mermaid/opencode-validate-mermaid.ts` to `~/.config/opencode/plugins/validate-mermaid.ts`, a dual-API module for OpenCode 2.x and for 1.x from 1.18.29. It does not write `plugins/rtk.ts`. OpenCode is not a spoke since it reads the hub through Claude compat; the wrappers add explicit `/` entries for skills.
